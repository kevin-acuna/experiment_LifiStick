...
//   Cada valor define un EJE COMPLETO: se recorre desde -INCLINATION_END
//   hasta +INCLINATION_END a lo largo de ese eje.
//   Internamente, el lado "negativo" usa azimuth+180° y el "positivo" usa azimuth.
//   Ejemplos:
//     {0.0}          -> eje 0°/180° completo (de -90° a +90° sobre ese eje)
...
int main()
{
    ...
    // Para cada eje, recorrer el barrido completo:
    //   Lado negativo: inc=INCLINATION_END → STEP con azimuth+180°
    //   Centro: inc=0° (entre lado negativo y positivo, solo una vez)
    //   Lado positivo: inc=STEP → INCLINATION_END con azimuth
    // Esto equivale a un barrido continuo de -INCLINATION_END a +INCLINATION_END.

    bool centerMeasured = false;

    for (int ai = 0; ai < (int)NUM_SCAN_AZIMUTHS; ai++) {
        double axis_az = SCAN_AZIMUTHS[ai];
        double opposite_az = fmod(axis_az + 180.0, 360.0);

        // Lado negativo: desde -INCLINATION_END hasta -STEP
        // (inclinación decreciente con azimuth opuesto)
        for (double inc = INCLINATION_END; inc >= INCLINATION_STEP - 1e-9; inc -= INCLINATION_STEP) {
            orientations.push_back({ inc, opposite_az });
        }

        // Centro (inclinación 0°): se mide una sola vez, entre lado negativo y positivo
        if (!centerMeasured) {
            orientations.push_back({ 0.0, 0.0 });
            centerMeasured = true;
        }

        // Lado positivo: desde +STEP hasta +INCLINATION_END
        // (inclinación creciente con azimuth original)
        for (double inc = INCLINATION_STEP; inc <= INCLINATION_END + 1e-9; inc += INCLINATION_STEP) {
            orientations.push_back({ inc, axis_az });
        }
    }
    ...
    cout << "  Ejes a escanear (barrido completo -" << INCLINATION_END << "° a +" << INCLINATION_END << "°): {";
    for (int ai = 0; ai < (int)NUM_SCAN_AZIMUTHS; ai++) {
        if (ai > 0) cout << ", ";
        cout << SCAN_AZIMUTHS[ai] << "°/" << fmod(SCAN_AZIMUTHS[ai] + 180.0, 360.0) << "°";
    }
    cout << "}\n";
